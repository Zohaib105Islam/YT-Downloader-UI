...
const YouTubePage = () => {
  ...
  const fetchVideoDetails = async () => {
    ...
    try {
      ...
      if (response.data.items?.length > 0) {
        ...
        setError("No YouTube video found.");
      }
    } catch (err) {
      setVideoData({
        snippet: {
  ...
  const handlePaste = async () => {
    try {
      ...
      setError("Failed to paste from clipboard on YouTube page.");
    }
  };
    ...
    <div>
      ...
      <main>
        <section className="search-box">
          ...
          <p>Download videos from YouTube with ease.</p>
          ...
          <button onClick={fetchVideoDetails}>Start YouTube Download</button>
        </section>
