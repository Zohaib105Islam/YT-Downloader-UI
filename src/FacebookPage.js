...
const FacebookPage = () => {
  ...
  const fetchVideoDetails = async () => {
    ...
    try {
      ...
      if (response.data.items?.length > 0) {
        ...
        setError("No Facebook video found.");
      }
    } catch (err) {
      setVideoData({
        snippet: {
  ...
  const handlePaste = async () => {
    try {
      ...
      setError("Failed to paste from clipboard on Facebook page.");
    }
  };
    ...
    <div>
      ...
      <main>
        <section className="search-box">
          ...
          <p>Download videos from Facebook with ease.</p>
          <div className="input-container">
            <input
              type="text"
              placeholder="Paste the Facebook video link here"
              value={videoUrl}
          ...
          <button onClick={fetchVideoDetails}>Start Facebook Download</button>
        </section>
